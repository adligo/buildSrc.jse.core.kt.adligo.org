...
object PipeDeps {
     ...
     */
    fun dependsOnPipe(gradleCallback : I_GradleCallback) {
        has(gradleCallback)
        gradleCallback.implementation(gradleCallback.projectFun("pipe.adligo.org"))
    }
    
    fun has(gradleCallback : I_GradleCallback) {
        I_PipeDeps.dependsOnI_Pipe(gradleCallback)
    }

    fun testsHave(gradleCallback : I_GradleCallback) {
        dependsOnPipe(gradleCallback)
    }
}
