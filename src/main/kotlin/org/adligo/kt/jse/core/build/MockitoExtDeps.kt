...
object MockitoExtDeps {

    /**
     * provides a way for other projects to depend on i_ctx
     */
    fun dependsOnMockitoExt(gradleCallback : I_GradleCallback) {
        has(gradleCallback)
        gradleCallback.implementation(gradleCallback.projectFun("mockito_ext.adligo.org"))
    }
    ...
    fun testsHave(gradleCallback : I_GradleCallback) {
        dependsOnMockitoExt(gradleCallback)
