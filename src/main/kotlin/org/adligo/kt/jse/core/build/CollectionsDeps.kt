...
object CollectionsDeps {

    /**
     * provides a way for other projects to depend on i_ctx
     */
    fun dependsOnCollections(gradleCallback : I_GradleCallback) {
        has(gradleCallback)
        gradleCallback.implementation(gradleCallback.projectFun("collections.adligo.org"))
    }
    ...
    fun testsHave(gradleCallback : I_GradleCallback) {
        dependsOnCollections(gradleCallback)
        CtxDeps.dependsOnCtx(gradleCallback)
